...
## 1. Canonical Brand Name Resolution (Â§0.2)
...
Derived directly from the reverse-engineered light theme in `settings_theme_switcher/code.html` (lines 66â€“158):
...
## 3. "Forgot Master Password" Security Architecture Decision (Â§6.3)

### The Security Dilemma
In a zero-knowledge local-first vault, vault records are encrypted at rest with keys derived via Argon2id from the user's master password. If the master password is forgotten, any cloud-based "password reset" mechanism would require storing an escrow decryption key on a remote serverâ€”violating the core promise that data never leaves the device in plaintext and has no backdoor.
...
## 4. Cryptographic Implementation Architecture (Â§6.1, Â§6.2)
...
4. **Device-to-Device Sync Handshake (Â§6.2):**
...
## 5. Multi-Platform Build & CI Matrix Reality (Â§0.1)
